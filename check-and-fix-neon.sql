-- Script pour créer toutes les tables dans Neon
-- Exécutez ce script dans Neon SQL Editor
-- Les erreurs "already exists" sont normales si les tables existent déjà

-- Créer l'extension si elle n'existe pas
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 3. Créer les tables manquantes (seulement si elles n'existent pas)
-- Table sessions
...
-- Table users (sans contrainte parent_host_id d'abord)
...
-- Ajouter la contrainte parent_host_id si elle n'existe pas
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'users_parent_host_id_fkey'
  ) THEN
    ALTER TABLE users ADD CONSTRAINT users_parent_host_id_fkey 
    FOREIGN KEY (parent_host_id) REFERENCES users(id) ON DELETE SET NULL;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'message_feedback_message_id_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'response_templates_user_id_fkey'
  ) THEN
    ALTER TABLE response_templates ADD CONSTRAINT response_templates_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'response_templates_property_id_fkey'
  ) THEN
    ALTER TABLE response_templates ADD CONSTRAINT response_templates_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'team_members_team_owner_id_fkey'
  ) THEN
    ALTER TABLE team_members ADD CONSTRAINT team_members_team_owner_id_fkey 
    FOREIGN KEY (team_owner_id) REFERENCES users(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'team_members_member_id_fkey'
  ) THEN
    ALTER TABLE team_members ADD CONSTRAINT team_members_member_id_fkey 
    FOREIGN KEY (member_id) REFERENCES users(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'team_members_property_id_fkey'
  ) THEN
    ALTER TABLE team_members ADD CONSTRAINT team_members_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'team_members_invited_by_fkey'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_staff_user_id_fkey'
  ) THEN
    ALTER TABLE cleaning_staff ADD CONSTRAINT cleaning_staff_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_tasks_property_id_fkey'
  ) THEN
    ALTER TABLE cleaning_tasks ADD CONSTRAINT cleaning_tasks_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_tasks_booking_id_fkey'
  ) THEN
    ALTER TABLE cleaning_tasks ADD CONSTRAINT cleaning_tasks_booking_id_fkey 
    FOREIGN KEY (booking_id) REFERENCES bookings(id) ON DELETE SET NULL;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_tasks_cleaning_staff_id_fkey'
  ) THEN
    ALTER TABLE cleaning_tasks ADD CONSTRAINT cleaning_tasks_cleaning_staff_id_fkey 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'special_requests_booking_id_fkey'
  ) THEN
    ALTER TABLE special_requests ADD CONSTRAINT special_requests_booking_id_fkey 
    FOREIGN KEY (booking_id) REFERENCES bookings(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'special_requests_property_id_fkey'
  ) THEN
    ALTER TABLE special_requests ADD CONSTRAINT special_requests_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'special_requests_cleaning_task_id_fkey'
  ) THEN
    ALTER TABLE special_requests ADD CONSTRAINT special_requests_cleaning_task_id_fkey 
    FOREIGN KEY (cleaning_task_id) REFERENCES cleaning_tasks(id) ON DELETE SET NULL;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'special_requests_responded_by_fkey'
  ) THEN
    ALTER TABLE special_requests ADD CONSTRAINT special_requests_responded_by_fkey 
    FOREIGN KEY (responded_by) REFERENCES cleaning_staff(id) ON DELETE SET NULL;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ical_sync_logs_property_id_fkey'
  ) THEN
    ALTER TABLE ical_sync_logs ADD CONSTRAINT ical_sync_logs_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'property_assignments_property_id_fkey'
  ) THEN
    ALTER TABLE property_assignments ADD CONSTRAINT property_assignments_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'property_assignments_cleaner_user_id_fkey'
  ) THEN
    ALTER TABLE property_assignments ADD CONSTRAINT property_assignments_cleaner_user_id_fkey 
    FOREIGN KEY (cleaner_user_id) REFERENCES users(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'property_assignments_assigned_by_fkey'
  ) THEN
    ALTER TABLE property_assignments ADD CONSTRAINT property_assignments_assigned_by_fkey 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_notes_property_id_fkey'
  ) THEN
    ALTER TABLE cleaning_notes ADD CONSTRAINT cleaning_notes_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_notes_cleaning_task_id_fkey'
  ) THEN
    ALTER TABLE cleaning_notes ADD CONSTRAINT cleaning_notes_cleaning_task_id_fkey 
    FOREIGN KEY (cleaning_task_id) REFERENCES cleaning_tasks(id) ON DELETE SET NULL;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_notes_author_id_fkey'
  ) THEN
    ALTER TABLE cleaning_notes ADD CONSTRAINT cleaning_notes_author_id_fkey 
    FOREIGN KEY (author_id) REFERENCES users(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaning_notes_resolved_by_fkey'
  ) THEN
    ALTER TABLE cleaning_notes ADD CONSTRAINT cleaning_notes_resolved_by_fkey 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cleaner_unavailability_cleaner_user_id_fkey'
  ) THEN
    ALTER TABLE cleaner_unavailability ADD CONSTRAINT cleaner_unavailability_cleaner_user_id_fkey 
    FOREIGN KEY (cleaner_user_id) REFERENCES users(id) ON DELETE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'blocked_periods_property_id_fkey'
  ) THEN
    ALTER TABLE blocked_periods ADD CONSTRAINT blocked_periods_property_id_fkey 
    FOREIGN KEY (property_id) REFERENCES properties(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'blocked_periods_host_id_fkey'
  ) THEN
    ALTER TABLE blocked_periods ADD CONSTRAINT blocked_periods_host_id_fkey 
...
DO $$ 
BEGIN
  RAISE NOTICE '✅ Script terminé ! Toutes les tables ont été créées ou vérifiées.';
END $$;
