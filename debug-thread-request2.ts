...
(async () => {
  ...
  const participants = thread?.participants?.edges?.map((edge: any) => ({
    accountId: edge?.node?.accountId,
    role: edge?.node?.participantRole,
  }));
  const messages = thread?.messages?.edges?.map((edge: any) => ({
    content: edge?.node?.contentPreview?.content,
    createdAtMs: edge?.node?.createdAtMs,
    accountId: edge?.node?.account?.accountId,
  }));
