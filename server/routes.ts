...
import {
  ...
  generateChatResponseWithFallback,
  extractAirbnbInfo,
  extractAirbnbInfoFromText,
} from "./gemini";
...
async function ensurePropertyAccess(req: any, res: any, next: any) {
  try {
    ...
    if (!userId) {
      logger.error("[PROPERTIES] ensurePropertyAccess: No user ID in request");
      logger.error("[PROPERTIES] Request path:", req.path);
      logger.error("[PROPERTIES] Is authenticated:", req.isAuthenticated());
      logger.error("[PROPERTIES] Session ID:", req.sessionID);
      return res.status(401).json({ error: "Authentication required" });
    }

    logger.info(
      `[PROPERTIES] ensurePropertyAccess: Checking access for user ${userId}`
    );
    ...
    try {
      ...
      logger.error(
        `[PROPERTIES] ensurePropertyAccess: Error fetching user ${userId}:`,
    ...
    if (!user) {
      logger.error(
        `[PROPERTIES] ensurePropertyAccess: User not found: ${userId}`
      );
      return res.status(404).json({ error: "User not found" });
    }

    logger.info(
      `[PROPERTIES] ensurePropertyAccess: User found: ${user.email}, role: ${user.role}`
    );
    ...
    try {
      ...
      logger.error(
        "ensurePropertyAccess: Error fetching properties:",
        propError?.message || propError
      );
      // Don't block access if we can't count properties - allow the operation
      logger.warn(
        "ensurePropertyAccess: Allowing access despite property count error"
      );
      return next();
    }
    ...
    return res.status(403).json({
      ...
      message:
        "Votre première propriété est gratuite ! Pour ajouter plus de propriétés, veuillez souscrire à un abonnement.",
      needsSubscription: true,
      currentPropertyCount: propertyCount,
    });
  } catch (error: any) {
    logger.error("Error checking property access:", error);
...
export async function registerRoutes(
  ...
  app.get("/api/health", async (req, res) => {
    try {
      ...
      const testUser = await storage.getUserByEmail(
        "health-check-test@test.com"
      );
      res.json({
        status: "ok",
  ...
  app.get("/api/properties", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      logger.info(`[PROPERTIES] Fetching properties for user: ${userId}`);

      const properties = await storage.getPropertiesByUser(userId);
      logger.info(
        `[PROPERTIES] Found ${properties.length} properties for user ${userId}`
      );

      res.json(properties);
    } catch (error: any) {
      logger.error(
        `[PROPERTIES] Error fetching properties for user ${req.user?.id}:`,
        error.message
      );
      res
        .status(500)
  ...
  app.get("/api/properties/:id", validateUUID("id"), async (req, res) => {
    try {
  ...
  app.get("/api/properties/by-key/:accessKey", async (req, res) => {
    try {
      const property = await storage.getPropertyByAccessKey(
  ...
  app.get("/api/reservation/:accessKey", async (req, res) => {
    try {
      ...
      logger.info(`[API] /api/reservation/${accessKey} - Starting lookup`);

      // 1. Chercher d'abord dans les réservations (bookings)
      let booking = await storage.getBookingByAccessKey(accessKey);
      logger.info(`[API] Booking found by accessKey:`, booking ? "yes" : "no");
      let property = null;
      ...
      if (booking) {
        ...
        logger.info(
          `[API] Property found by booking.propertyId:`,
          property ? "yes" : "no"
        );
      } else {
        ...
        logger.info(
          `[API] Property found by accessKey:`,
          property ? "yes" : "no"
        );
        ...
        if (property) {
          ...
          logger.info(
            `[API] Active booking for property:`,
            booking ? "yes" : "no"
          );

          // Si pas de booking, créer un booking virtuel pour la compatibilité
          if (!booking) {
            logger.info(`[API] Creating virtual booking`);
            booking = {
      ...
      if (!property || !booking) {
        logger.info(
          `[API] Reservation not found - property:`,
          !!property,
          "booking:",
          !!booking
        );
        return res.status(404).json({
          ...
          message:
            "Ce lien n'est pas valide ou a expiré. Veuillez contacter votre hôte.",
        });
      }

      // 3. Calculer la logique de déblocage 24h avant le check-in
      const now = new Date();
      const defaultCheckIn = now;
      const defaultCheckOut = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);

      // Parser les dates de manière sécurisée avec fallback
      const checkInDate = parseSafeDate(booking.checkInDate, defaultCheckIn);
      const checkOutDate = parseSafeDate(booking.checkOutDate, defaultCheckOut);

      logger.info(
        `[API] Dates parsed - checkIn: ${checkInDate}, checkOut: ${checkOutDate}`
      );
      ...
      const hoursUntilUnlock = Math.max(
        0,
        Math.ceil((unlockDateTime.getTime() - now.getTime()) / (1000 * 60 * 60))
      );

      let unlockMessage = "";
      if (isUnlocked) {
        unlockMessage =
          "Bienvenue ! Toutes les informations d'accès sont disponibles.";
      } else if (now < unlockDateTime) {
        unlockMessage = `Les informations d'arrivée seront disponibles le ${unlockDateTime.toLocaleDateString(
          "fr-FR",
          {
            weekday: "long",
            day: "numeric",
        ...
        )} à ${unlockDateTime.toLocaleTimeString("fr-FR", {
          hour: "2-digit",
          minute: "2-digit",
        })}.`;
      } else {
      ...
      logger.error(
        "[API] Error fetching reservation:",
        error?.message || error
      );
  ...
  app.get(
    "/api/arrival-eligibility/:propertyId",
    ...
    async (req, res) => {
      try {
        ...
        const activeBooking = await storage.getActiveBookingForProperty(
          propertyId
        );
        ...
        if (!activeBooking) {
          ...
          const hasArrivalInfo =
            property.arrivalMessage || property.arrivalVideoUrl;
          return res.json({
            eligible: false,
            reason: hasArrivalInfo
              ? "no_active_booking"
              : "no_booking_and_no_arrival_info",
            hasArrivalInfo: hasArrivalInfo,
            checkInDate: null,
            checkOutDate: null,
            message:
              "Les informations d'arrivée seront disponibles la veille de votre check-in.",
          });
        }
        ...
        logger.error("Error checking arrival eligibility:", error);
...
END:VCALENDAR`;
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${property.name.replace(
          /[^a-zA-Z0-9]/g,
          "_"
        )}.ics"`
      );
      ...
      logger.error("Error exporting iCal:", error);
  ...
  app.get(
    "/api/properties/:id/ical-export-url",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const { id } = req.params;
        ...
        res.json({
          ...
          instructions: {
            ...
            forCleaners:
              "Partagez ce lien avec vos agents de ménage pour qu'ils puissent voir le calendrier des réservations",
          },
          note: "Ce lien est permanent et n'expire jamais. Il fonctionne sans authentification.",
        });
      } catch (error) {
        logger.error("Error getting iCal export URL:", error);
  ...
  app.post(
    "/api/properties/:id/regenerate-ical-token",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const { id } = req.params;
        ...
        logger.error("Error regenerating iCal token:", error);
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        const userId = req.user.id;
        logger.info(`[PROPERTIES] Creating property for user: ${userId}`);

        const validatedData = insertPropertySchema.parse(req.body);
        const property = await storage.createProperty(validatedData, userId);

        logger.info(
          `[PROPERTIES] Property created: ${property.id} (${property.name}) for user ${userId}`
        );
          ...
          try {
            ...
            if (subscription.items.data.length > 0) {
              await stripe.subscriptions.update(user.stripeSubscriptionId, {
                items: [
                  { id: subscription.items.data[0].id, quantity: newCount },
                ],
              });
            }
          } catch (stripeError) {
            logger.error("Error updating Stripe quantity:", stripeError);
        ...
        logger.error(
          `[PROPERTIES] Error creating property for user ${req.user?.id}:`,
          error.message
        );
        res.status(400).json({
  ...
  app.get("/api/conversations/property/:propertyId", async (req, res) => {
    try {
      const conversations = await storage.getConversationsByProperty(
    ...
    try {
      ...
      if (!allowedHosts.includes(parsedUrl.hostname.toLowerCase())) {
        return res.status(400).json({
          error:
            "Veuillez fournir un lien Airbnb valide (airbnb.com, airbnb.fr, etc.)",
        });
      }
      ...
      logger.error("Airbnb import error:", error);
      res.status(500).json({
        error:
          error.message ||
          "Impossible d'importer les informations de l'annonce Airbnb",
      });
    }
  });
    ...
    async (req: any, res) => {
      ...
      const importTimeout = setTimeout(() => {
        if (!res.headersSent) {
          res.status(408).json({
            ...
            message:
              "L'import a pris trop de temps. Veuillez réessayer ou utiliser la méthode manuelle (copier-coller le texte).",
          });
        }
      ...
      try {
        ...
        try {
          ...
          logger.error("Playwright scraping failed:", e?.message);
        ...
        if (
          ...
          try {
            ...
            logger.error("AI extraction failed:", aiError?.message);
        ...
        try {
          ...
          logger.warn(
            "Could not count properties, allowing import anyway:",
            countError?.message
          );
        ...
        if (propertyCount > 0) {
          try {
            ...
            logger.warn(
              "Could not check subscription, allowing import anyway:",
              subError?.message
            );
        ...
        clearTimeout(importTimeout);
        logger.error("Error importing Airbnb property:", error);
        logger.error("Error stack:", error?.stack);
        ...
        const message =
          error?.message || "Impossible d'importer la propriété depuis Airbnb";
        ...
        if (message.includes("timeout") || message.includes("Timeout")) {
          return res.status(408).json({
            error: "Import timeout",
            message:
              "L'import a pris trop de temps. Veuillez utiliser la méthode manuelle (copier-coller le texte de la page).",
          });
        }
        ...
        if (message.includes("browser") || message.includes("Playwright")) {
          return res.status(500).json({
            ...
            message:
              "L'import automatique a échoué. Veuillez utiliser la méthode manuelle (copier-coller le texte de la page).",
            details: message,
          });
        }
  ...
  app.post(
    "/api/properties/import-airbnb-text",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.id;
        ...
        if (
          ...
          return res
            .status(400)
            .json({ error: "Texte insuffisant pour l'extraction" });
        }
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        if (propertyCount === 0) {
          return res.status(400).json({
            error: "You must have at least one property to subscribe",
          });
        }
        ...
        if (!priceId) {
          return res.status(400).json({
            error:
              "Subscription price not configured. Please set STRIPE_PRICE_PER_PROPERTY_ID",
          });
        }
        ...
        res.json({
          ...
          clientSecret: (subscription.latest_invoice as any)?.payment_intent
            ?.client_secret,
          propertyCount,
          pricePerProperty: 29.9,
          totalPrice: propertyCount * 29.9,
        });
      } catch (error: any) {
  ...
  app.post("/api/demo-chat", async (req, res) => {
    try {
      ...
      const demoProperty = await storage.getPropertyByAccessKey(
        "demo-paris-01"
      );
      if (!demoProperty) {
        logger.error("Demo property not found with accessKey: demo-paris-01");
      ...
      if (!process.env.GEMINI_API_KEY) {
        ...
        return res.status(500).json({
          error:
            "AI service is not configured. Please check GEMINI_API_KEY in .env file.",
        });
      }
      ...
      logger.error("Error in demo chat:", error);
      logger.error("Error stack:", error?.stack);
      ...
      const errorMessage =
        error?.message || String(error) || "Failed to generate response";
      res.status(500).json({
  ...
  app.get("/api/feedback/stats", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error?.message || "Failed to fetch feedback stats" });
    }
  });
  ...
  app.post("/api/templates", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.id;
      const validated = insertResponseTemplateSchema.parse({
        ...req.body,
        userId,
      });
  ...
  // Export routes
  app.get(
    "/api/conversations/export/:propertyId",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.id;
        ...
        if (format === "csv") {
          // Export CSV
          const csvRows = [
            "Conversation ID,Guest Name,Message ID,Is Bot,Content,Language,Category,Created At",
          ];
          ...
          res.setHeader(
            "Content-Disposition",
            `attachment; filename=conversations-${propertyId}-${Date.now()}.csv`
          );
          ...
          res.setHeader(
            "Content-Disposition",
            `attachment; filename=conversations-${propertyId}-${Date.now()}.json`
          );
          res.json(exportData);
        } else {
          res
        ...
        res
          .status(500)
          .json({ error: error?.message || "Failed to export conversations" });
      }
    }
  );
  ...
  app.post("/api/team/members", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res
        .status(400)
        .json({ error: error?.message || "Failed to create team member" });
    }
  });
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ error: error?.message || "Failed to delete team member" });
      }
    }
  );
    ...
    try {
      ...
      if (property.userId) {
        await storage.createNotification({
          ...
          content: `Un voyageur a déclenché une alerte SOS pour ${
            property.name
          }. Message: ${message || "Pas de détails fournis"}`,
          metadata: {
            propertyId,
            alertType: "sos",
            timestamp,
      ...
      logger.info(`🚨 SOS Alert for property ${property.name}: ${message}`);
  ...
  app.post("/api/widget-chat", async (req, res) => {
    try {
      ...
      if (!propertyId || !message) {
        return res
          .status(400)
          .json({ error: "Property ID and message required" });
      }
      ...
      const response = await generateChatResponseWithFallback(
        message,
        property
      );
      ...
      res.status(500).json({
        error: "Chat error",
        response:
          "Désolé, je ne peux pas répondre pour le moment. Veuillez contacter l'hôte directement.",
      });
    }
  });
      ...
      `;
        ...
        conversations.forEach((conv, idx) => {
          ...
          <div class="conversation">
            ...
            <p class="meta">ID: ${conv.id} | Créée le: ${new Date(
            conv.createdAt
        ...
        `;
          ...
          messages.forEach((msg) => {
            ...
            <div class="message ${msg.isBot ? "bot" : "user"}">
              ...
              <p>${msg.content}</p>
              <span class="meta">${new Date(msg.createdAt).toLocaleTimeString(
                "fr-FR"
              )}</span>
            </div>
          `;
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=conversations-export-${Date.now()}.html`
        );
        res.send(html);
      } catch (error: any) {
  ...
  function safeParseJson(data: unknown): any | null {
    try {
      if (typeof data !== "string" && !(data instanceof Buffer)) return null;
  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    // TODO: Implement real WebSocket auth (e.g. token or session-based)
